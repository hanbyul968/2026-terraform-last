...
variable "competitor_number" {
  description = "?€??ë¹„ë²ˆ??(?? 01, 15). S3 ë²„í‚· ?´ë¦„ wsc-msk-order-data-<ë¹„ë²ˆ??-bucket ???¬ìš©"
  type        = string
}
...
resource "aws_iam_role_policy" "ec2_s3" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # ì±„ì /ê²€ì¦??¸ì˜: EC2?ì„œ Lambda ESM ?íƒœ ?•ì¸Â·enable, DynamoDB ì¡°íšŒ
        Effect = "Allow"
        Action = [
...
# Bastion IAM Role (admin + ? ë¢°?•ì±… ?„ì²´ ?¤í”ˆ)
...
# Application EC2 (Aì¡? public subnet)
resource "aws_instance" "app" {
  ...
  # user_data ë³€ê²???stop/start(?¬ì‹¤?????????„ë‹ˆ???¸ìŠ¤?´ìŠ¤ êµì²´ ????user_dataê°€ ê¹¨ë—???¤í–‰
...
# kafka-python ?¤ì¹˜ (producer.py/ec2_consumer.py ê°€ ?°ëŠ” ?¼ì´ë¸ŒëŸ¬ë¦?
python3 -m pip install kafka-python-ng boto3

# ??ë¨¼ì?: ec2_consumer.py / producer.py ?¤ìš´ë¡œë“œ (?‘ê³  ë¹ ë¦„ ??kafka tarball ê¸°ë‹¤ë¦¬ì? ?ŠìŒ)
...
# Kafka CLI ?´ë¼?´ì–¸??? í”½ ?ì„±?????ë¦° archive.apache.org ??ë°±ê·¸?¼ìš´?œë¡œ ë°›ì•„ user_data ë¸”ë¡œ??????(
  for i in 1 2 3; do
...
# S3 Bucket (?´ë¦„: wsc-msk-order-data-<ë¹„ë²ˆ??-bucket)
...
# VPC Lambda??ENI ?ì„± ê¶Œí•œ???ì„± ?œì ???„ìš” ??ê´€ë¦¬í˜• ?•ì±… ë¶€ì°?resource "aws_iam_role_policy_attachment" "lambda_vpc" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole"
  role       = aws_iam_role.lambda.name
}

# IAM ?•ì±… ?„íŒŒ ì§€??eventual consistency) ?€ê¸???Lambda ?ì„± ??ENI ê¶Œí•œ ?•ì‹¤???ìš©
