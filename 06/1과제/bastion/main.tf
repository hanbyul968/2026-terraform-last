...
#           를 수행한다. (로컬에 Docker 불필요)
#   - 접속: SSM Session Manager (SSH 키/인바운드 불필요)
#   - 권한: 인스턴스 프로파일(AdministratorAccess)로 terraform 자격증명 자동 사용
#   - 자동화: 루트(../) 1과제 코드(modules/docker/manifest 포함)를 zip 으로 묶어
#            부트스트랩 S3 에 업로드 → user_data 가 내려받아 /opt/task1 에 준비.
#            SSM 접속 후 `bash /opt/task1/run.sh` 한 줄로 main 인프라가 배포된다.
#
...
#  - source_dir : 상위 1과제 폴더(현재 로컬 파일; modules/docker/manifest 포함)
#  - 제외       : bastion 폴더, .terraform, state, lock, plan (리눅스에서 새로 init)
#  - 번들 zip 은 1과제 폴더 "밖"(06/)에 생성하여 자기 자신을 포함하지 않게 한다.
# =============================================================================
data "archive_file" "task1" {
  type        = "zip"
  source_dir  = "${path.module}/.."
  output_path = "${path.module}/../../.task1_bundle_06.zip"
