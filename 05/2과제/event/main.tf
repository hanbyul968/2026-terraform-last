...
# ─────────────────────────────────────────────
# IAM Role: EC2 (CloudWatch + SSM 권한)
# ─────────────────────────────────────────────
...
# systemd 서비스 등록: gj2026-app
# Restart=no → 장애 시 자동 재시작 안 함 (알람 트리거 목적)
...
ExecStart=/usr/local/bin/uvicorn app:app --host 127.0.0.1 --port 8080 --log-config /home/ec2-user/log_config.yaml
...
# uvicorn 로그 설정 (로그 파일 기록)
cat > /home/ec2-user/log_config.yaml << 'LOGEOF'
version: 1
...
chown ec2-user:ec2-user /home/ec2-user/log_config.yaml

systemctl daemon-reload
systemctl enable gj2026-app
systemctl start gj2026-app

# SSM Parameter Store 백업 (초기)
sleep 5
...
# CW Agent 설정: 앱 로그 + 커스텀 메트릭(app_process_count)
...
# 10초 간격 cron
echo "* * * * * root /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
echo "* * * * * root sleep 10 && /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
echo "* * * * * root sleep 20 && /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
echo "* * * * * root sleep 30 && /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
echo "* * * * * root sleep 40 && /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
echo "* * * * * root sleep 50 && /usr/local/bin/put-app-metric.sh" >> /etc/cron.d/app-metric
EOF
...
# ─────────────────────────────────────────────
# CloudWatch Alarm: app_process_count < 1
# ─────────────────────────────────────────────
...
resource "aws_cloudwatch_metric_alarm" "app" {
  ...
  alarm_actions = [
    ...
    "arn:aws:events:ap-northeast-2:${data.aws_caller_identity.current.account_id}:event-bus/default"
  ]
}

# ─────────────────────────────────────────────
# SSM Parameter Store: /gj2026/event/app-py
# (초기 빈 값 - EC2 userdata에서 실제 값으로 덮어씀)
# ─────────────────────────────────────────────
resource "aws_ssm_parameter" "app_py" {
  name  = "/gj2026/event/app-py"
  type  = "String"
  value = "# placeholder - replaced by EC2 userdata"

  lifecycle {
    ignore_changes = [value] # EC2에서 실제 값으로 업데이트되므로 terraform이 덮어쓰지 않게
  }
}
...
# ─────────────────────────────────────────────
# EventBridge Rule: Alarm → recovery Lambda
# ─────────────────────────────────────────────
resource "aws_cloudwatch_event_rule" "alarm_trigger" {
  name        = "gj2026-event-trigger-alarm"
  description = "CloudWatch Alarm ALARM 상태 → recovery Lambda 실행"
