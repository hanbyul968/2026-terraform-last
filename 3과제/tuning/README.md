...
>
> **request는 실측 사용량에서 벗어나지 않는다.** 파드당 실사용(동시 총 CPU p95 ÷ 파드수)의 절반이
> 하한이고, 한 회차에 현재값의 절반 아래로는 내리지 않는다. 1800m을 쓰는 파드에 50m을 예약하는
> 비현실적인 값이 나오지 않게 하는 장치다. 더 내려야 하면 실측 후 다음 회차가 이어서 내린다.
>
...
- **탐색보다 나눗셈이 먼저다.** 실측 사용량에서 request를 한 번에 역산한다.
  `request = 동시 총 CPU p95 ÷ 파드수`. 500m 예약에 250m만 쓰면 250m, 750m에 파드당 1800m을 쓰면
  올린다(과소예약). 이건 시험이 아니라 계산이므로 한 회차 측정으로 끝난다.
- 같은 방식으로 **필요 노드 수와 앱별 예약을 즉시 계산**한다.
  `필요 노드 = ceil((시스템 예약 + 앱 동시 CPU p95 합) / allocatable)`,
  `앱별 request = (필요 노드 × allocatable − 시스템 예약) × CPU 비중 ÷ 파드수`.
  실측 평균 노드가 이미 그 값이면 **request로는 비용을 더 줄일 수 없다**고 바로 알리고(`cost_locked`),
  노드 감소를 노린 후보는 시험하지 않는다. 3회차를 낭비하지 않기 위한 장치다.
  실측 예(2026-08-20 19시): 수요 11298m + 시스템 900m → 7대 = 관측 7.0대 → 비용 절감 불가.
- 선형 지연 모델을 신뢰할 수 있는 범위는 제한적이므로 한 회차 외삽은 공급 부족 1.5배까지만
  허용한다. 더 내려가야 하면 그 값을 120초 실측한 뒤 다음 회차가 이어서 판단한다.
- **실측 CPU 수요보다 공급이 적어지는 노드 수는 제안하지 않는다.** 실측(2026-08-20): 4노드(공급
  7720m < 수요 11122m)에서 가용성 게이트 실패, 6~7노드는 정상. 단 하한을 채우려고 request를
  올리지는 않는다(현재 예약이 더 낮으면 현재 노드 수가 하한).
...
# 실제 탐색: 기본 18분 예산 안에서 HPA를 패치하며 공식 총점을 최대화(워밍업+120s, 예산 초과 시 자동 중단)
.\optimize.ps1 -Apply
