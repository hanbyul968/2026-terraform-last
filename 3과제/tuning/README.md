...
### max_replicas 는 비용 예산에서 역산한다

`max_replicas` 를 감으로 크게 열면 안 된다. HPA 는 부하가 요구하면 상한까지 파드를 만들고
Karpenter 가 노드를 공급하므로 **천장이 곧 비용**이 된다.

실측이다. `max` 를 6 → 30 으로 열자 성능은 +0.5, 비용은 **-4.0** 이었다.

| 회차 | 가용성 | 성능 | 비용 | 소계 | 노드평균 |
|---|---|---|---|---|---|
| `max 6` | 12.0 | 6.0 | **8.0** | **26.0** | 3.82 |
| `max 30` | 12.0 | 6.0 | 7.0 | 25.0 | 4.41 |
| `max 30` | 12.0 | **6.5** | **4.0** | 22.5 | 5.62 |

그래서 `advise.py` 는 `--cost-points`(기본 8)로 **비용에서 확보할 점수**를 받아 상한을 역산한다.
채점기준이 고정이라 이 역산은 앱·트래픽이 바뀌어도 유효하다.

```
비용 점수  ->  허용 ratio  ->  허용 노드 평균  ->  (노드CPU - 시스템) / request  =  max_replicas
```

```
비용 현황: 노드 평균 5.62대 -> ratio 2.81 -> 4/12점
    목표 8점을 받으려면 노드 평균 4.0대 이하 (ratio <= 2.00)
[stress] [!] max 30 는 비용 예산(8점, 노드 4.0대)을 넘긴다 -> max 21 로 제한 (request 300m 기준)
```

노드 타입·기준 노드 수·앱 request 가 달라지면 상한도 따라 움직인다.

| 조건 | 노드 예산 | max |
|---|---|---|
| t3.medium, 기준 2대, request 300m | 4.0대 | 21 |
| t3.medium, 기준 3대, request 300m | 6.0대 | 34 |
| 큰 노드(3860m), 기준 2대, request 300m | 4.0대 | 47 |
| t3.medium, 기준 2대, request 50m | 4.0대 | 130 |

### 성능 1구간과 비용 1구간의 교환비

성능·가용성 구간은 **0.5점**, 비용 구간은 **1.0점**이다. 그리고 비용 구간은 노드 평균 **0.5대마다**
바뀐다. 즉 노드 한 대를 줄이면 2점인데, 성능 한 구간은 0.5점이다.

`advise.py` 는 회차들을 채점 총점으로 줄세워 어느 방향이 이득인지 보여준다.

```
회차 비교 (채점 소계 = 가용성+성능+비용, 비정상요청 4점 제외)
회차               가용성     성능     비용     소계   노드평균
max6              12.0     6.0     8.0    26.0      3.82  <== 최고
max30-a           12.0     6.0     7.0    25.0      4.41
max30-b           12.0     6.5     4.0    22.5      5.62
```

성능을 짜서 얻는 0.5점보다 노드를 줄여 얻는 1~2점이 큰 경우가 많다. **성능이 30% 위에 있는 한**
비용을 먼저 보는 것이 유리하다(30% 아래로 가면 비용 12점이 통째로 0 이 되므로 그 선은 절대 지킨다).

### min_replicas — HPA 램프를 없애는 가장 값싼 수단

HPA 는 사후 반응이라 **트래픽이 계단처럼 들어오면 항상 램프 구간이 생긴다.** 이 과제는 T+1h 에
트래픽이 시작되므로 그 구간의 지연·실패가 누적 로그에 그대로 박힌다(실측: min 2 → 8 까지 **1분 45초**).

`min` 을 올리면 그 구간이 사라진다. 공짜는 아니지만 **아래 부등식 안에서는 노드가 늘지 않아 비용이
그대로다.**

```
Σ(min_i × request_i) + 시스템요청  ≤  기준노드수 × 노드당 할당가능
```

`advise.py` 가 매 실행마다 이 상한을 계산해 출력한다.

```
min 여유: 기준 2노드(3860m) - 시스템 600m = 3260m 여유
    -> 노드를 늘리지 않고 각 앱 min 을 최대 5 까지 올릴 수 있다 (현재 합계 6)
```

숫자를 코드에 박지 않는 이유는 앱이 바뀌면 `request` 가 바뀌어 상한도 바뀌기 때문이다.
노드 타입·노드 수·앱 개수가 달라져도 같은 부등식으로 다시 계산된다.

### scale_up 은 늦추지 않는다

`scale_up` 은 쿠버네티스 기본값(`stabilization 0s`, `Percent 100/15s`, `Pods 4/15s`, `Max`)을 쓴다.
한때 노드 폭증을 막으려 `stabilization 30s` + `Percent 50` + `Pods 2` 로 **일부러 느리게** 잡아뒀는데,
그것이 램프 구간을 늘려 성능 점수를 깎았다. 스케일업을 늦춰서 얻는 것은 없다 — 플래핑 억제는
`scale_down` 의 90s 창이 담당하고, 늘어난 파드는 부하가 빠지면 되돌려진다. 비용 위험도 없다:
스케일업은 사용률이 요구할 때만 일어난다.

### 반대로 손대지 않는 것 — readinessProbe

새 파드가 트래픽을 받는 시점을 실제로 결정하는 것은 readinessProbe 가 아니라 **ALB** 다.

| 단계 | 소요 |
|---|---|
| `readinessProbe` (period 5s, initialDelay 0) | ~5초 |
| ALB health check (`interval 10s` × `healthy_threshold 2`) | **최대 20초** |

`initialDelaySeconds` 는 이미 0 이고 period 를 5→2 로 줄여도 뒤에 ALB 20초가 있어 체감이 없다.
줄이려면 ALB `interval` 을 낮춰야 하는데 그건 오탐 위험과 맞바꾸는 것이고, 앱 기동 특성에 의존해서
앱 무관 개선이 아니다. 그래서 건드리지 않는다.
