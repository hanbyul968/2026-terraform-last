...
# ========== Bastion ==========
# NOTE: 인-메인 bastion(EC2/IAM/SG/SSM/AMI 파라미터)은 1단계 스테이지(bastion/ 서브폴더)로
# 분리되었다. 로컬 PowerShell 에서 `cd bastion && terraform apply` 로 Bastion 을 띄우고,
# SSM 접속 후 그 안에서 이 루트(main) 를 apply 한다. 루트의 다른 리소스가 기존 bastion
# 리소스를 참조하지 않음을 확인하고 제거했다. 기존 bastion-userdata.sh 는 더 이상 루트에서
# 참조되지 않으며 bastion/userdata.sh.tpl 이 이를 대체한다.
