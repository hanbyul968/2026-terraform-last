# =============================================================================
# 1?¨ê³„ (ë¡œì»¬ Windows PowerShell ?ì„œ apply) ??2ê³¼ì œ ë°°í¬ ?„ìš© Bastion EC2
#   - ëª©ì : Windows ë¡œì»¬ ?€??Linux Bastion ?ˆì—??2ê³¼ì œ 4ê°?ëª¨ë“ˆ(module1~4 +
#           module1/k8s)??terraform apply ?œë‹¤. (PowerShell ?˜ì¡´ ì½”ë“œ ?œê±° ??#           ëª¨ë“  provisioner/?¤í¬ë¦½íŠ¸ê°€ bash ë¡??™ì‘)
#   - ?‘ì†: SSM Session Manager (SSH ???¸ë°”?´ë“œ ë¶ˆí•„??
#   - ê¶Œí•œ: ?¸ìŠ¤?´ìŠ¤ ?„ë¡œ?Œì¼(AdministratorAccess)ë¡?terraform ?ê²©ì¦ëª… ?ë™ ?¬ìš©
#   - ?ë™?? ?ìœ„(../) 2ê³¼ì œ ì½”ë“œ ?„ì²´ë¥?zip ?¼ë¡œ ë¬¶ì–´ ë¶€?¸ìŠ¤?¸ë© S3 ???…ë¡œ????#            user_data ê°€ ?´ë ¤ë°›ì•„ /opt/task2 ???€ê³?/opt/task2/deploy.sh ?ì„±.
#            SSM ?‘ì† ??`bash /opt/task2/deploy.sh` ??ì¤„ë¡œ ?„ì²´ ë°°í¬ê°€ ?˜í–‰?œë‹¤.
#
# ?????´ë”(state)??module1~4 state ?€ ?„ì „??ë¶„ë¦¬?˜ì–´ ?ˆë‹¤. ì±„ì  ?????´ë”?ì„œë§?#   `terraform destroy` ?˜ë©´ Bastion(+ë¶€?¸ìŠ¤?¸ë© ë²„í‚·)ë§??œê±°?œë‹¤.
# ??archive ??ëª¨ë“  state/backup/.terraform/lock/plan ???œì™¸?œë‹¤(ë¦¬ëˆ…?¤ì—???ˆë¡œ init).
#   ë¡œì»¬??terraform.tfstate*.backup ?Œì¼?¤ì? "?ˆë? ?? œ?˜ì? ?Šê³ " zip ?ë§Œ ?£ì? ?ŠëŠ”??
# =============================================================================
...
# =============================================================================
# ?ìœ„(2ê³¼ì œ) ì½”ë“œ ë²ˆë“¤ë§???ë¶€?¸ìŠ¤?¸ë© S3 ?…ë¡œ??#  - source_dir : ?ìœ„ 2ê³¼ì œ ?´ë”(module1~4, manifest, k8s, *.py, *.zip, layer ?¬í•¨)
#  - ?œì™¸       : bastion ?´ë”, .terraform, ëª¨ë“  state/backup, lock, plan
#  - ë²ˆë“¤ zip ?€ 2ê³¼ì œ ?´ë” "ë°?(09/)???ì„±?˜ì—¬ ?ê¸° ?ì‹ ???¬í•¨?˜ì? ?Šê²Œ ?œë‹¤.
# =============================================================================
