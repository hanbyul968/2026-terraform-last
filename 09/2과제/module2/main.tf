...
# setup.sh Î•??¥Îäî S3 Î≤ÑÌÇ∑ (bastion??Î∞õÏïÑ ?§Ìñâ)
...
# ?Ä?Ä Bastion EC2 (Windows Î°úÏª¨ ?Ä??VPC ?¥Î??êÏÑú kubectl/helm ?§Ìñâ) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
# private endpoint ?¥Îü¨?§ÌÑ∞??VPC Î∞?Windows Î°úÏª¨)?êÏÑ† kubectl Î∂àÍ? ??bastion?êÏÑú ?òÌñâ
data "aws_ami" "al2023" {
  most_recent = true
  owners      = ["amazon"]
  filter {
...
# S3?êÏÑú setup.sh Î∞õÏïÑ ?òÎ®∏ÏßÄ Î¶¨ÏÜå??ALB Controller, Loki/Grafana/FluentBit/nginx, Ingress) ?êÎèô Íµ¨ÏÑ±
...
# bastion role ??EKS cluster-admin (kubectl Í∂åÌïú)
...
# bastion ??EKS API 443 (?¥Îü¨?§ÌÑ∞ SGÍ∞Ä VPC CIDR 443?Ä ?¥Î? ?àÏö©?òÏ?Îß?Î™ÖÏãú)
...
# ALB??Ingress(logging-ingress) + AWS Load Balancer ControllerÍ∞Ä ?ùÏÑ±?úÎã§.
# (terraform?êÏÑú ALB/TG/Î¶¨Ïä§?àÎ? ÎßåÎì§Î©?Í∞ôÏ? ?¥Î¶Ñ?ºÎ°ú Ï§ëÎ≥µ ?ùÏÑ±?òÏñ¥ DuplicateLoadBalancerName Ï∂©Îèå)
# ???§Ï†ú ALB ?ùÏÑ±¬∑?åÎìú IP ?ÄÍπ??±Î°ù¬∑Í≤ΩÎ°ú ?ºÏö∞?ÖÏ? k8s IngressÍ∞Ä ?¥Îãπ. README 5?®Í≥Ñ Ï∞∏Í≥†.

# CloudShell/bastion(VPC ?¥Î?) ??EKS API ?ëÍ∑º ?àÏö© (private cluster Ï±ÑÏ†ê??
