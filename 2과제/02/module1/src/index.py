...
def calculate_grade(average):
    # 평균 점수를 기반으로 등급 반환: A(90~100) B(80~89) C(70~79) D(60~69) F(0~59)
    if average >= 90:
        return "A"
    if average >= 80:
        return "B"
...
def save_student(table, row):
    # 5과목 평균 계산 → 등급 산출 → DynamoDB 저장
    scores = {field: int(row[field].strip()) for field in SCORE_FIELDS}
    average = (Decimal(sum(scores.values())) / Decimal(len(SCORE_FIELDS))).quantize(Decimal("0.1"))
    item = {
