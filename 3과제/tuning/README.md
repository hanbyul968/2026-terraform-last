...
> **목표는 비용도 성능도 아니고 총점이다.** 비용 12점과 성능·가용성 24점은 맞바꿀 수 있으므로,
> 한쪽을 목적함수로 두면 반대쪽에서 더 큰 점수를 흘린다. 엔진은 '노드 수 → 총점' 곡선을 그려
> **총점이 최대인 운영점**을 고르고, 후보 탐색은 그 지점을 겨냥한다(`plan()["frontier"]`).
>
> **비용 ratio의 분모(B)는 비공개다. 그래서 하나로 찍지 않는다.**
> 다행히 40점 중 성능+가용성 24점은 **B와 완전히 무관**하다. B가 흔드는 건 비용 12점뿐이다.
> 그래서 곡선의 '품질' 축은 확정값으로 그릴 수 있고, 불확실성은 비용 축에만 남는다.
> `--cost-baselines`(기본 `2,3,4`)로 준 후보들에서 모두 평가한 뒤 두 값을 함께 낸다.
>
> - `expected_nodes` — B 균등 가중 **기대 총점**이 최대인 노드 수
> - `minimax_nodes` — 어떤 B가 진짜여도 **최악 손해(regret)가 가장 작은** 노드 수 ← 추천값
>
> B별 최적이 모두 같으면 `robust: true`로 보고한다. 즉 **분모를 몰라도 답이 확정**인 경우가 많다.
>
> **하한 절벽 주의: `ratio < 0.50`이면 비용 12점이 통째로 0이다.** 그래서 "노드가 적을수록 유리"가
> 성립하지 않고, 저구간에서는 노드를 늘리는 쪽이 이득이다. 프론티어를 Pareto 가지치기로 줄이면
> 안 되는 이유이며, 엔진은 구간 전체를 평가한다.
>
> 곡선을 눈으로 보려면: `python optimize.py <outdir> --frontier`
>
...
품질은 15.5→16.0(+0.5점)뿐인데 비용은 최대 9.5점을 잃는다 — **성능이 CPU가 아니라 DB에
묶여 있다는 신호**이므로 노드를 사면 안 된다. 반대로 품질 열이 노드에 따라 크게 오르면
(CPU 바운드) 비용 몇 점을 내주고 노드를 늘리는 쪽이 총점에서 이긴다. 그 판단을 엔진이 한다.
...
- **성능/가용성 게이트 복구가 비용보다 절대 우선이다.** 어떤 앱이 유지선(성능 80%·가용성 90%)
  밑이면 그 앱의 `gate-recovery`가 최상위 후보로 나온다. 미달이 심할수록(예: user 14.8%) HPA
  target을 크게 내리고(최저까지) min/max를 늘려 파드를 일찍·많이 확장해 경합·지연을 줄인다.
  성능이 30% 밑이면 비용 12점이 통째로 0이 되므로, 비용을 아끼려 성능을 30% 밑으로 떨구는 조합은
  절대 채택하지 않는다. gate-recovery는 HPA만 바꿔 부하 중에도 즉시 적용된다.
