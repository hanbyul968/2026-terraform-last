...
# Pillow 필요 → 사전에 lambda/rotate_pkg/ 에 pip install 필요
# ─────────────────────────────────────────────
...
resource "aws_lambda_function" "rotate" {
  ...
  runtime          = "python3.12" # PDF 명세: python3.14 → AWS에서 지원 시 python3.14 로 변경
...
resource "aws_lambda_function" "request" {
  ...
  publish          = true # Lambda@Edge는 반드시 publish 필요
  timeout          = 5
...
# ─────────────────────────────────────────────
# CloudFront Cache Policy (image + rotate 쿼리 파라미터 기준 캐시 분리)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
locals {
  # Lambda Function URL에서 https:// 와 trailing / 제거
...
resource "aws_cloudfront_distribution" "cdn" {
  ...
  # Origin: Lambda Function URL (rotate)
  origin {
