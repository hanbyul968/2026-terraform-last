...
output "next_steps" {
  ...
  value       = <<-EOT
    ...
    3) main 인프라 한 번에 배포 (docker build/push 포함):
         bash /opt/task1/run.sh

    4) 채점 직전(★): 로컬 PowerShell에서 Bastion만 제거 (8-2 감점 방지)
         cd bastion; terraform destroy -var="player_id=${var.player_id}" -auto-approve
    ─────────────────────────────────────────────────────────────
  EOT
}
