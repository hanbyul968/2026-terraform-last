...
2. ▶ 부하 시작 클릭 (브라우저에서 측정·채점)
3. 테스트 완료 후 📊 채점 클릭

### 🚀 서버 고부하 (노드 스케일용)

브라우저 `fetch`는 오리진당 동시 연결이 **~6개(HTTP/1.1)**로 막혀 있어, Concurrency를
아무리 올려도 실제 in-flight 요청은 6개뿐이라 **노드가 잘 안 늘어난다**. 노드(HPA/Karpenter)를
실제로 스케일시킬 만큼 강하게 부하를 주려면 **🚀 서버 고부하** 버튼을 쓴다.

- 요청을 브라우저가 아니라 **`server.py`가 스레드풀로 직접** 쏜다(브라우저 연결 제한과 무관).
- Concurrency=워커 스레드 수, 테스트 시간=지속시간, 요청 간격=워커당 대기(0=최대속도)를 그대로 사용.
- 실측: 워커 40개로 **~1,600 req/s** (응답 빠른 대상 기준). 워커를 늘리면 더 올라간다.
- 상단 「▶ 부하 시작」(측정·채점)과 **완전히 분리** — 점수 로직엔 영향 없이 부하만 준다.
- 상태 표시줄에 누적 요청/초당 요청(req/s)이 뜨고, 설정한 시간이 지나면 자동 종료.
