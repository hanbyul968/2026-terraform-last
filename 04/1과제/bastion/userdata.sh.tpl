...
#!/bin/bash
set -e
# 1단계 (AWS 레이어): VPC/KMS/S3/CloudFront/WAF/ECR(빌드)/DynamoDB/Lambda/EKS/노드그룹/ALB+TG/IAM/PodIdentity
cd /opt/task1
terraform init -input=false
terraform apply -auto-approve
# 2단계 (k8s/helm 레이어): Namespace/SA/ConfigMap/Deployment/Service/StorageClass,
#   AWS LB Controller(helm), book TargetGroupBinding(kubectl),
#   fluent-bit(helm), prometheus/grafana(helm)+PVC+dashboard, wsc-addon-ingress,
#   그리고 마지막에 EKS public->private 전환(finalize)
cd /opt/task1/k8s
terraform init -input=false
terraform apply -auto-approve
echo ""
