...
# ë¡œì»¬ PC(Windows)?ì„œ ?¤í–‰?˜ë©°, Docker/Terraform???¤ì¹˜??Linux Bastion???„ìš´??
# Bastion?ëŠ” ../main êµ¬ì„±ê³?app ë°”ì´?ˆë¦¬ê°€ ?…ë¡œ?œë˜ê³? ê´€ë¦¬ìž ê¶Œí•œ ?¸ìŠ¤?´ìŠ¤
# ?„ë¡œ?Œì¼??ë¶€?¬ëœ?? ?¤ì œ ì±„ì  ?€???¸í”„??skills-book-*)??Bastion ?ˆì—??# `terraform apply` ë¡??ì„±?œë‹¤.
###############################################################################
...
# --- Bastion ?„ìš© ìµœì†Œ ?¤íŠ¸?Œí¬ (ì±„ì  ?€??skills-book-vpc ?€ ë¶„ë¦¬) ---------
...
# --- Bastion IAM (ê´€ë¦¬ìž ê¶Œí•œ: main ?¤í…Œ?´ì? ?„ì²´ ë¦¬ì†Œ?¤ë? ?ì„±) -------------
...
resource "aws_instance" "bastion" {
  ...
  # 1) main ?¤í…Œ?´ì?(ì±„ì  ?€??êµ¬ì„± + app ë°”ì´?ˆë¦¬)ë¥?Bastion?¼ë¡œ ?…ë¡œ??  provisioner "file" {
    source      = "${path.module}/../main"
    destination = "/home/ec2-user"
  }

  # 2) Docker / Terraform ?¤ì¹˜ + tfvars ?‘ì„± + apply ?¬í¼ ?ì„±
