...
- **자동 탐색(권장)**: `.\optimize.ps1 -Apply` — warmup 60초, baseline 120초 후 최대 3개 후보를
  각각 120초 실측한다. 기본 목표는 **비용 우선**이다. 공식 채점상 가용성은 `≥90%`면 앱당 만점이고
  성능은 `<30%`에서만 비용이 0이 되므로, 지켜야 할 선은 **가용성 ≥92% · 성능 ≥35%**로 두고 그 안에서
  노드를 줄인다. 성능을 우선하려면 `-Objective balanced`(가용성 99% 유지).
